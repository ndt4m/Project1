...
public class Table {
    ...
    protected void syncRecord(String baseId, String token) {
        ...
        if (records == null) {
            System.out.print("Error: Could not get records for table: " + name);
        } else {
    ...
    protected boolean updateField(JsonObject newField, Field field, String baseId, String token) {
        ...
        if (fieldUpdate == null) {
            System.out.print("Error: Could not update field: " + field.getName() + " in table: " + name);
            return false;
        }
        JsonObject fieldJson = JsonParser.parseString(fieldUpdate).getAsJsonObject();
        fields.remove(field);
        fields.add(new Field(fieldJson));
        System.out.print("Updated field: " + field.getName() + " in table: " + name);
        return true;
    }
    ...
    protected boolean addField(JsonObject field, String baseId, String token) {
        ...
        if (fieldCreate == null) {
            System.out.print("Error: Could not create field: " + field.get("name").getAsString() + " in table: " + name);
            return false;
        }
        JsonObject fieldJson = JsonParser.parseString(fieldCreate).getAsJsonObject();
        fields.add(new Field(fieldJson));
        System.out.print("Created field: " + field.get("name").getAsString() + " in table: " + name);
        return true;
    }
    ...
    private boolean updateRecord(JsonObject fields, Record record, String baseId, String token) {
        ...
        if (recordUpdate == null) {
            System.out.print("Error: Could not update record: " + record.getValOfId() + " in table: " + name);
            return false;
        }
        JsonObject recordJson = JsonParser.parseString(recordUpdate).getAsJsonObject();
        records.remove(record);
        records.add(new Record(recordJson));
        System.out.print("Updated record: " + record.getValOfId() + " in table: " + name);
        return true;
    }
    ...
    private boolean addRecord(JsonObject fields, String baseId, String token) {
        ...
        if (recordCreate == null) {
            System.out.print("Error: Could not create record: " + fields.get("Id").getAsString() + " in table: " + name + "has id: " + id + " baseId: " + baseId);
            return false;
        }
        ...
        System.out.print("Created record: " + fields.get("Id").getAsString() + " in table: " + name);
        return true;
    }
    ...
    private boolean pullRecord(JsonObject fields, String baseId, String token) {
        ...
        if (oldRecord == null) {
            if (addRecord(fields, baseId, token)) {
                System.out.print("Add record: " + fields.get("Id").getAsString() + " in table: " + name);
                numChanges++;
                return true;
            }
            return false;
        }
        if (oldRecord.equals(this.fields)) {
            return true;
        }
        if (updateRecord(fields, oldRecord, baseId, token)) {
            System.out.print("Update record: " + fields.get("Id").getAsString() + " in table: " + name);
            numChanges++;
            return true;
        }
        return false;
    ...
    protected boolean pullAllRecord(List<JsonObject> fields, String baseId, String token) {
        ...
        for (JsonObject field : fields) {
            if (!pullRecord(field, baseId, token)) {
                System.out.print("Error: Could not pull record: " + field.get("Id").getAsString() + " in table: " + name);
                return false;
            }
        }
        System.out.print("Pulled all records in table: " + name);
        return true;
    }
        ...
        for (Record record : this.records) {
            ...
            if (!isExist) {
                if (Record.dropRecord(record.getId(), id, baseId, token)) {
                    System.out.print("Deleted record: " + record.getValOfId() + " in table: " + name);
                    dropList.add(record);
                } else {
                    System.out.print("Error: Could not delete record: " + record.getValOfId() + " in table: " + name);
                }
            }
        }
    ...
    protected static String listTables(String baseId, String token) {
        ...
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            if (response.getCode() != 200) {
                System.out.print("Error: Could not list tables");
                return null;
            }
            System.out.print("Listed tables");
            return EntityUtils.toString(response.getEntity());
        } catch (IOException | ParseException e) {
            System.out.print("Error: Could not list tables due to exception: " + e.getMessage());
            return null;
        }
    }
    ...
    protected static String createTable(String name, JsonArray fields, String baseId, String token) {
        ...
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            if (response.getCode() != 200) {
                System.out.print("Error: Could not create table: " + name);
                return null;
            }
            System.out.print("Created table: " + name);
            return EntityUtils.toString(response.getEntity());
        } catch (IOException | ParseException e) {
            System.out.print("Error: Could not create table: " + name + " with message: " + e.getMessage());
            return null;
        }
    }
}
